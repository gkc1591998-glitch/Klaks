...
class Category extends CI_Controller {
	...
	public $addPage  = 'category-add';	   
	public $editPage = 'category-edit';   
	public $listPage = 'category';		  

	public $listPage_redirect = '/admin/Category';		 
	public $addPage_redirect  = '/admin/Category/add/';	
	public $editPage_redirect = '/admin/Category/edit/';  


	public function __construct() {
    parent::__construct();
  	$this->load->model('Category_model','my_model');  
	...
	public function add()
	{
		...
		if($this->input->post('submit') != '')
		{
			...
			if($exit_details == 0)
			{
				...
				{
					// No image uploaded, pass empty string
					$ImgData = '';
					$result = $this->my_model->add_record($ImgData);
					if($result){
						$this->session->set_flashdata('msg_succ', 'Inserted Successfully...');
		...
		if($this->input->post('submit') != '')
		{
			...
			if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
				...
				if(!$this->upload->do_upload('image')){
					...
					if (!empty($old_image) && file_exists(FCPATH.'images/category/'.$old_image)) {
	...
	public function top_category_status($id,$status){
		$data['msg'] ='';
		$statu = ($status == 1 ? 'Deactive' : 'Active');
		if($id){
			$result = $this->my_model->top_category_status_record($id,$status);
			if($result){
				$this->session->set_flashdata('msg_succ', $statu.' Successfully...');
				redirect($this->listPage_redirect);
			}else{
